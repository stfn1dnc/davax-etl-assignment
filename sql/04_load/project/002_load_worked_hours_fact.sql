...
(
    ...
    LEFT JOIN target.dim_employee employee
        ON LOWER(TRIM(employee.employee_id)) =
           LOWER(TRIM(raw.email_raw))

    JOIN target.dim_date date_dimension
        ON date_dimension.full_date =
           TO_DATE(
               raw.date_worked_raw DEFAULT NULL
               ON CONVERSION ERROR,
               'YYYY-MM-DD'
           )
    ...
    WHERE raw.worked_hours_raw IS NOT NULL
